    ...
    test(new FourBitAdder).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      for(a <- 0 to 15){
        for(b<- 0 to 15){
          ...
          val sum = (a+b) % 16// converting to binary
          val cout = if (a + b >= 16) 1 else 0
          dut.io.a.poke(a.U)		// inputs
          dut.io.b.poke(b.U)
          dut.io.sum.expect(sum.U)	// expected outputs
          dut.io.carry.expect(cout.U)
        }
      }
    }
  }
